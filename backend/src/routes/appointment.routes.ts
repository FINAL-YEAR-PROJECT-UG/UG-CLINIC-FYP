...
router.get('/all', authenticate, (req, res, next) => {
  ...
  req.user && ['RECEPTIONIST', 'DOCTOR', 'ADMIN'].includes(req.user.role) ? next() : res.status(403).json({ success: false, message: 'Forbidden: staff access only' });
}, getAllAppointments);

router.patch('/:id/assign', authenticate, (req, res, next) => {
  req.user && ['RECEPTIONIST', 'ADMIN'].includes(req.user.role) ? next() : res.status(403).json({ success: false, message: 'Forbidden: receptionist/admin only' });
}, assignDoctorToAppointment);
